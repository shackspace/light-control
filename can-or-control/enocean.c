...
void enocean_init(void) {
	...
	for (uint8_t i = 0; i < ENOCEAN_CHANNEL_COUNT; i++)
	{
		...
		enocean_channel_state[i] = eeprom_read_byte((const uint8_t *)ENOCEAN_CHANNEL_EEPROM_STORE+i);
...
void enocean_main(void) {
	...
	for (uint8_t i = 0; i < ENOCEAN_CHANNEL_COUNT; i++) {
		...
		if (enocean_channel_state[i] & ENOCEAN_CHANNEL_EEPROM)
		{
			...
			eeprom_write_byte((uint8_t *)ENOCEAN_CHANNEL_EEPROM_STORE+i, enocean_channel_state[i] & ENOCEAN_CHANNEL_STATUS);
		}
